...
public class AclImpl extends OwnerImpl implements Acl, Serializable {
    ...
    // or principal.
    //
    public HashSet<PrincipalEntryPair> allowedUsers = new HashSet<PrincipalEntryPair>(23);
    public HashSet<PrincipalEntryPair> allowedGroups = new HashSet<PrincipalEntryPair>(23);
    public HashSet<PrincipalEntryPair> deniedUsers = new HashSet<PrincipalEntryPair>(23);
    public HashSet<PrincipalEntryPair> deniedGroups = new HashSet<PrincipalEntryPair>(23);
    @JsonIgnore
    ...
    public HashSet<PrincipalEntryPair> getAllowedUsers() {
        return allowedUsers;
    }

    public HashSet<PrincipalEntryPair> getAllowedGroups() {
        return allowedGroups;
    }

    public HashSet<PrincipalEntryPair> getDeniedUsers() {
        return deniedUsers;
    }
            ...
            throws NotOwnerException
    {
        ...
        HashSet<PrincipalEntryPair> aclTable = findTable(entry);
        Principal key = entry.getPrincipal();

        if (getEntry(key, aclTable) != null)
            return false;

        PrincipalEntryPair pe = new PrincipalEntryPair();
        pe.entry = entry;
        pe.principal = key;
        aclTable.add(pe);
        return true;
            ...
            throws NotOwnerException
    {
        ...
        AclEntry o = removeEntry(key, aclTable);//aclTable.remove(key);
        return (o != null);
    }

    private AclEntry removeEntry(Principal p, HashSet<PrincipalEntryPair> aclTable){
        Iterator<PrincipalEntryPair> it =  aclTable.iterator();
        AclEntry entry = null;
        while (it.hasNext()){
            PrincipalEntryPair pe = it.next();
            if(pe != null && pe.principal.equals(p)) {
                it.remove();
                entry = pe.entry;
            }
        }
    ...
    private AclEntry getEntry(Principal p, HashSet<PrincipalEntryPair> aclTable){
        Iterator<PrincipalEntryPair> it =  aclTable.iterator();
        AclEntry entry = null;
        while (it.hasNext()){
            PrincipalEntryPair pe = it.next();
    ...
    //
    private HashSet<PrincipalEntryPair> findTable(AclEntry entry) {
        HashSet<PrincipalEntryPair> aclTable = null;

        Principal p = entry.getPrincipal();
        if (p instanceof Group) {
            if (entry.isNegative())
                aclTable = deniedGroups;
            else
                aclTable = allowedGroups;
        } else {
            if (entry.isNegative())
                aclTable = deniedUsers;
            else
                aclTable = allowedUsers;
        }
        return aclTable;
    }
    ...
    private Enumeration<Permission> getGroupPositive(Principal user) {
        ...
        Enumeration<Principal> e = Collections.enumeration(
                allowedGroups.stream().map(pair -> {
                    return pair.principal; }).collect(Collectors.toSet())
        );
        while (e.hasMoreElements()) {
            Group g = (Group)e.nextElement();
            if (g.isMember(user)) {
                AclEntry ae = getEntry(g, allowedGroups);
    ...
    private Enumeration<Permission> getGroupNegative(Principal user) {
        ...
        Enumeration<Principal> e = Collections.enumeration(
                deniedGroups.stream().map(pair -> {
                    return pair.principal; }).collect(Collectors.toSet())
        );
        while (e.hasMoreElements()) {
            Group g = (Group)e.nextElement();
            if (g.isMember(user)) {
                AclEntry ae = getEntry(g, deniedGroups);
    ...
    private Enumeration<Permission> getIndividualPositive(Principal user) {
        ...
        AclEntry ae = getEntry(user, allowedUsers);
        if (ae != null)
    ...
    private Enumeration<Permission> getIndividualNegative(Principal user) {
        ...
        AclEntry ae  = getEntry(user, deniedUsers);
        if (ae != null)
